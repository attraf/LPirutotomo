...
const translations = {
  ko: {
    ...
    reservationNotPossible: {
      ...
      desc: "다음의 경우 예약이 불가능합니다. 예약 불가가 확인된 경우 전액 환불해 드립니다. (이메일로 환불 상세 내용 확인 가능)",
      reasons: [
        "예약을 받지 않는 가게",
    ...
    cancellationPolicy: {
      title: "취소 정책",
      desc: "예약 완료 전까지 취소 및 환불 신청이 가능합니다. 예약 완료 이메일 수신 후 취소는 불가능합니다."
    },
    importantInfo: {
      title: "상세 정보",
      points: [
  ...
  ja: {
    ...
    reservationNotPossible: {
      title: "予約ができない場合もあるの？",
      desc: "次の場合は予約ができません。予約不可が確認された場合は全額ご返金致します。(メールにて払い戻しの詳細を確認可能)",
      reasons: [
        "予約を受け付けていないお店",
    ...
    cancellationPolicy: {
      title: "キャンセルポリシー",
      desc: "予約完了の前まで、キャンセル及び払い戻しの申請が可能です。予約完了メール受け取り後のキャンセルは不可です。"
    },
    importantInfo: {
      title: "詳細情報",
      points: [
      ...
      <div className="p-4 space-y-6">
        ...
        <section>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="font-bold">{t.howToReserve.title}</h2>
          </div>
          <Card className="p-4 bg-white/50">
            <ol className="list-none space-y-3">
              {t.howToReserve.steps.map((step, index) => (
                <li key={index} className="flex gap-2">
                  <span className="text-[#00CBB3]">{index === 0 ? "1️⃣" : index === 1 ? "2️⃣" : "3️⃣"}</span>
        ...
        <section>
          <div className="flex items-center gap-2 mb-3">
            <HelpCircle className="h-5 w-5 text-[#00CBB3]" />
            <h2 className="font-bold">{t.reservationNotPossible.title}</h2>
          </div>
          <Card className="p-4 bg-white/50">
            <p className="text-sm mb-3">{t.reservationNotPossible.desc}</p>
            <ol className="list-none space-y-3">
              {t.reservationNotPossible.reasons.map((reason, index) => (
                <li key={index} className="flex gap-2">
                  <span className="text-[#00CBB3]">{index === 0 ? "1️⃣" : "2️⃣"}</span>
        ...
        <section>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="font-bold">{t.cancellationPolicy.title}</h2>
          </div>
          <Card className="p-4 bg-white/50 border-2 border-[#FFA500]">
            <p className="text-sm">
              {t.cancellationPolicy.desc}
            </p>
          </Card>
        </section>
        ...
        <section>
          <div className="flex items-center gap-2 mb-3">
            <AlertTriangle className="h-5 w-5 text-[#FFA500]" />
            <h2 className="font-bold">{t.importantInfo.title}</h2>
          </div>
          <Card className="p-4 bg-white/50 space-y-4">
            {t.importantInfo.points.map((point, index) => (
        ...
        <section>
          <Card className="p-4 bg-white/50 border-2 border-[#00CBB3]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold mb-1">{t.contact.title}</h3>
                <p className="text-sm text-[#00CBB3]">{t.contact.email}</p>
              </div>
            </div>
          </Card>
        </section>
